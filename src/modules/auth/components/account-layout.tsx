...
export default function AccountLayout({ 
  ...
  const menuItems = [
    { id: 'balance', label: 'Số dư', icon: WalletIcon, extra: `${Number(user.balance).toLocaleString()}đ` },
    { id: 'proxies', label: 'Proxy', icon: ShieldCheckMarkIcon },
    { id: 'orders', label: 'Đơn hàng', icon: OrderIcon, badge: orders.length > 0 ? orders.length : undefined },
    { id: 'payments', label: 'Giao dịch', icon: CashDollarIcon },
    { id: 'profile', label: 'Hồ sơ', icon: PersonIcon },
  ];
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto flex gap-8 py-8 px-4">
        ...
        <aside className="w-48 flex-shrink-0 space-y-0.5">
          {menuItems.map((item) => (
            ...
            >
            <Icon source={item.icon} tone={activeTab === item.id ? 'inherit' : 'subdued'} />
            ...
            <div className="space-y-4">
               ...
               <div className="flex flex-wrap gap-1.5 mb-6">
                  {[
                    ...
                    { label: 'Xuất file', icon: ExportIcon }
                  ].map(btn => (
            ...
            <div className="max-w-2xl space-y-8">
              <div className="bg-white border border-slate-200 rounded-xl p-6 flex items-center justify-between shadow-sm">
                 ...
                 <div className="p-3 bg-blue-50 rounded-lg">
                    <Icon source={WalletIcon} tone="primary" />
                 </div>
              </div>
              ...
              <div className="space-y-6">
                 ...
                 <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                       {[
                         { id: 'bank', label: 'Ngân hàng', icon: CreditCardIcon },
                         { id: 'momo', label: 'Ví Momo', icon: MobileIcon },
                         { id: 'crypto', label: 'Crypto', icon: FinanceIcon },
                         { id: 'card', label: 'Thẻ cào', icon: BankIcon },
                       ].map(method => (
                         <div key={method.id} className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 hover:border-blue-500 hover:bg-blue-50/10 cursor-pointer transition-all group">
                            <Icon source={method.icon} tone="subdued" />
